...
– Evaluates models → metrics_eth.csv
– Proposes top‐K combos, re‐backtests on test slice, and only keeps those with sufficient entries → top_*_eth.csv
...
in ml_way3.py limiter will be settine earliest 1460 days (4 years) of data. last 1 years will be untouched .
earliest 1460 will be divided and used for training and testing.
The last 1 year will be used for testing manually as an complete unseen data by me in the jupyter notebook ml_way.ipynb
"""
...
# ── Config ──────────────────────────────────────────────────────
DATA_5M          = "data/ETHUSDT_5m_5year.csv"
DATA_1H          = "data/ETHUSDT_1h_5year.csv"
LIMITER_DAYS     = 1460
TRAIN_PCT        = 0.7
N_SAMPLES        = 500
TOP_K            = 5
...
def limit_to_first_n_days(df, n):
     # build sorted list of unique dates, then take the first n of them
     days = df['datetime'].dt.normalize().drop_duplicates().sort_values()
     first = days.iloc[:n]
     # keep only rows whose date is in that earliest-n set
     return df[df['datetime'].dt.normalize().isin(first)].reset_index(drop=True)
...
def main():
    ...
    df_samp.to_csv('ml_training_data_filtered_eth.csv', index=False)
    ...
    pd.DataFrame(metrics).to_csv('metrics_eth.csv', index=False)
    ...
    for name, mdl in models.items():
        ...
        pd.DataFrame(out).to_csv(f'top_{name}_combos_filtered_eth.csv', index=False)

    print("Done. Check:")
    print(" • ml_training_data_filtered_eth.csv")
    print(" • metrics_eth.csv")
    print(" • top_<model>_combos_filtered_eth.csv")
