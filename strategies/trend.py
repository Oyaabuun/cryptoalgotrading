#!/usr/bin/env python3
"""
Trend‐regime breakout strategy (5m + 1h only) with full CPU utilization on macOS:
- Parallel hyperparameter grid search using all cores
- VWAP band confluence (±0.1%)
- Volume spike filter
- Session filter (excl. 00:00 UTC)
- Dynamic stops & partial scale‐out
- Uses ProcessPoolExecutor for speed
"""
...
INITIAL_USDT   = 58.0
LEVERAGE       = 10.0
CONTRACT_SZ    = 0.001    # micro‐contract size (minQty & stepSize = 0.001)
TAKER_FEE      = 0.0005
SLIPPAGE       = 0.0005
VWAP_BAND_PCT  = 0.001    # ±0.1%
EXCLUDE_HOURS  = {0}      # skip midnight hour
...
def _evaluate_combo(combo, df5_train, df1h_train):
    # worker to run a single combo and return metrics
    hf = preprocess_1h(df1h_train, combo)
    b5 = preprocess_5m(df5_train.copy(), combo, hf)
    _, mets = run_backtest(b5, combo)
    return {**combo, **mets}


def run_strategy(df_5m_train, df_1h_train, df_5m_test, df_1h_test):
    # 1) Parallel grid‐search on TRAIN slice
    combos = [dict(zip(param_grid, vals)) for vals in product(*param_grid.values())]
    results = []
    with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
        future_to_combo = {
            executor.submit(_evaluate_combo, combo, df_5m_train, df_1h_train): combo
            for combo in combos
        }
    ...
    grid_df.to_csv('trend_best_parameters.csv', index=False)

    # 2) pick best by Sharpe
    best = grid_df.loc[grid_df['sharpe_ratio'].idxmax()]
    best_params = {k: int(best[k]) if k in ['ATR_WINDOW','EMA_LONG'] else float(best[k])
                   for k in param_grid}
    print("Best hyperparameters:", best_params)

    # 3) Backtest on TRAIN with best params
    hf_b = preprocess_1h(df_1h_train, best_params)
    b5_b = preprocess_5m(df_5m_train.copy(), best_params, hf_b)
    bt_tr, bt_met = run_backtest(b5_b, best_params)
    bt_tr.to_csv('trend_backtest_with_best_para.csv', index=False)
    pd.DataFrame([bt_met]).to_csv('trend_backtest_results.csv', index=False)

    # 4) Test‐run on TEST
    hf_t = preprocess_1h(df_1h_test, best_params)
    b5_t = preprocess_5m(df_5m_test.copy(), best_params, hf_t)
    ts_tr, ts_met = run_backtest(b5_t, best_params)
    ts_tr.to_csv('trend_testrun_with_best_para.csv', index=False)
    pd.DataFrame([ts_met]).to_csv('trend_testrun_results.csv', index=False)
